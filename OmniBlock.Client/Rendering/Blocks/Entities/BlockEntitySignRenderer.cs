...
public class BlockEntitySignRenderer : BlockEntitySpecialRenderer
{
    ...
    public void renderTileEntitySignAt(BlockEntitySign sign, double x, double y, double z, float tickDelta)
    {
        ...
        float rotationYaw;
        if (signBlock == tileEntityRenderer.World.Content.Blocks.Get("sign"))
        {
            RenderSystem.ModelView.Translate((float)x + 0.5F, (float)y + 12.0F / 16.0F * modelScale, (float)z + 0.5F);
            var rotationDegrees = sign.PushedBlockData * 360 / 16.0F;
            RenderSystem.ModelView.Rotate(-rotationDegrees, 0.0F, 1.0F, 0.0F);
        ...
        else
        {
            ...
            RenderSystem.ModelView.Translate((float)x + 0.5F, (float)y + 12.0F / 16.0F * modelScale, (float)z + 0.5F);
            RenderSystem.ModelView.Rotate(-rotationYaw, 0.0F, 1.0F, 0.0F);
            RenderSystem.ModelView.Translate(0.0F, -(5.0F / 16.0F), -(7.0F / 16.0F));
        ...
        RenderSystem.ModelView.Translate(0.0F, 0.5F * modelScale, 0.07F * modelScale);
        RenderSystem.ModelView.Scale(rotationYaw, -rotationYaw, rotationYaw);
        RenderSystem.Normal = new Vector3D<float>(0.0F, 0.0F, -1.0F * rotationYaw);
