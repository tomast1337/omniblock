...
public class FluidsRenderer : IBlockRenderer
{
    public bool Draw(Block block, in BlockPos pos, ref BlockRenderContext ctx)
    {
        ...
        var tintR = ((colorMultiplier >> 16) & 255) / 255.0F;
        var tintG = ((colorMultiplier >> 8) & 255) / 255.0F;
        var tintB = (colorMultiplier & 255) / 255.0F;

        // Determine which faces are actually visible to the player
        var isTopVisible = block.IsSideVisible(ctx.BlockReader, pos.X, pos.Y + 1, pos.Z, Side.Up);
        var isBottomVisible = block.IsSideVisible(ctx.BlockReader, pos.X, pos.Y - 1, pos.Z, 0);
        bool[] sideVisible =
        [
        ...
        var meta = ctx.BlockReader.GetBlockMeta(pos.X, pos.Y, pos.Z);

        // Calculate the height of the fluid at each of the 4 corners of this block
        var heightNw = GetFluidVertexHeight(ref ctx, pos.X, pos.Y, pos.Z, material);
        var heightSw = GetFluidVertexHeight(ref ctx, pos.X, pos.Y, pos.Z + 1, material);
        var heightSe = GetFluidVertexHeight(ref ctx, pos.X + 1, pos.Y, pos.Z + 1, material);
        var heightNe = GetFluidVertexHeight(ref ctx, pos.X + 1, pos.Y, pos.Z, material);

        // TOP FACE (Flowing Surface)
        if (ctx.RenderAllFaces || isTopVisible)
        {
            hasRendered = true;
            var textureId = block.GetTexture(Side.Up, meta);
            var flowAngle = (float)FluidMath.GetFlowingAngle(ctx.BlockReader, pos.X, pos.Y, pos.Z, material);
            ...
            var sinAngle = MathHelper.Sin(flowAngle) * 0.5F;
            var cosAngle = MathHelper.Cos(flowAngle) * 0.5F;
        ...
        for (var side = 0; side < 4; ++side)
        {
            ...
            var textureId = block.GetTexture((side + 2).ToSide(), meta);
            ctx.Tess.setArrayLayer(Atlases.Terrain.LayerOfGridIndex(textureId));
            ...
            if (ctx.RenderAllFaces || sideVisible[side])
            {
                ...
                var minV1 = 1.0F - h1; // UV height match for corner 1
                var minV2 = 1.0F - h2; // UV height match for corner 2
                const float maxV = 1.0F;

                ctx.SetLightAt(block, adjX, pos.Y, adjZ);
                var shadow = side < 2 ? lightZ : lightX;
    ...
    private float GetFluidVertexHeight(ref BlockRenderContext ctx, int x, int y, int z, Material material)
    {
        ...
        for (var i = 0; i < 4; ++i)
        {
            ...
            var neighborMaterial = ctx.BlockReader.GetMaterial(checkX, y, checkZ);
            ...
            {
                var neighborMeta = ctx.BlockReader.GetBlockMeta(checkX, y, checkZ);
                var fluidDepth = FluidMath.GetFluidHeightFromMeta(neighborMeta);
