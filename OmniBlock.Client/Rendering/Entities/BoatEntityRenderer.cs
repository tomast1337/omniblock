...
public class BoatEntityRenderer : EntityRenderer
{
    ...
    public void render(Entity boatEntity, double x, double y, double z, float yaw, float tickDelta)
    {
        ...
        {
            RenderSystem.ModelView.Rotate(MathHelper.Sin(timeSinceHit) * timeSinceHit * damageTaken / 10.0F * hull.RockDirection(boatEntity), 1.0F, 0.0F, 0.0F);
        }

        loadTexture("/terrain.png");
        var modelScale = 12.0F / 16.0F;
        RenderSystem.ModelView.Scale(modelScale, modelScale, modelScale);
        RenderSystem.ModelView.Scale(1.0F / modelScale, 1.0F / modelScale, 1.0F / modelScale);
