...
if [[ ( "$requested_scenario" == "chunk-visibility-baseline" || "$requested_scenario" == "entity-render-baseline" || "$requested_scenario" == "entity-lod-selection" || "$requested_scenario" == entity-impostor-* ) && -z "${E2E_TIMEOUT_SECONDS:-}" ]]; then
    timeout_seconds=300
fi
