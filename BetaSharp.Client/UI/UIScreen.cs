...
public abstract class UIScreen
{
    ...
    public UIScreen(UIContext context)
    {
        ...
        CreateButton = () => new Button(context.PlayClickSound);
        CreateImageButton = () => new ImageButton(context.PlayClickSound);
        CreateSlider = () => new Slider(context.PlayClickSound);
    }
    ...
    public void Initialize()
    {
        ...
        if (!_isInitialized)
        {
            ...
            {
                ControlTooltipBar tooltipBar = new(Context, this);
    ...
    protected virtual void PreLayout(float scaledWidth, float scaledHeight) => _titleSpacer?.Style.Height = Math.Max(0f, scaledHeight / 4f - _titleSpacerOffset);

    public virtual void Uninit() => Keyboard.enableRepeatEvents(false);
    ...
    public void HandleInput()
    {
        ...
        {
            HandleSliderEditTick();
        }
    }

    private void HandleControllerScroll()
    {
        if (!Context.ControllerState.IsControllerMode)
        {
            return;
        }
    ...
    private void HandleSliderEditTick()
    {
        ...
        if (lx != 0f)
        {
            ...
            while (_sliderStickAccumulated >= 1f)
            {
                _editingSlider.AdjustValue(step);
                _sliderStickAccumulated -= 1f;
            }
    ...
    {
        if (!element.Visible || !element.IsHitTestVisible)
        {
            return;
        }
        ...
        if (element.OnClick == null && element.OnMouseDown == null)
        {
            return;
        }

        if (element.ComputedWidth <= 0 || element.ComputedHeight <= 0)
        {
            return;
        }
    ...
    public virtual void Update(float partialTicks) => Root.Update(partialTicks);
    ...
    private void UpdateHovers(float mouseX, float mouseY)
    {
        ...
        if (newHovered != _hoveredElement)
        {
            if (_hoveredElement != null)
            {
                ...
                _hoveredElement.OnMouseLeave?.Invoke(new UIMouseEvent
                {
            ...
            if (_hoveredElement != null && _hoveredElement.Enabled)
            {
                ...
                _hoveredElement.OnMouseEnter?.Invoke(new UIMouseEvent
                {
                    Target = _hoveredElement,
                    MouseX = (int)mouseX,
                    MouseY = (int)mouseY
    ...
    private void UpdateCursors()
    {
        if (Context.Options.UICursors)
        {
            if (_hoveredElement is Button btn)
            {
                if (btn.Enabled)
                {
                    Mouse.SetClickCursor();
                }
                else
                {
                    Mouse.SetDisabledCursor();
                }
            }
            else if (_hoveredElement is ListItem || _hoveredElement is Link)
            {
                Mouse.SetClickCursor();
            }
            else if (_hoveredElement is TextField)
            {
                Mouse.SetTextCursor();
            }
            else
