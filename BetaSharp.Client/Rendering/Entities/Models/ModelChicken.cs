...
public class ModelChicken : ModelBase
{
    ...
    public override void render(float yOffset, float limbSwingAmount, float ageInTicks, float headYaw, float headPitch, float scale)
    {
        setRotationAngles(yOffset, limbSwingAmount, ageInTicks, headYaw, headPitch, scale);
        head.render(scale);
        bill.render(scale);
        chin.render(scale);
        body.render(scale);
    ...
    public override void setRotationAngles(float yOffset, float limbSwingAmount, float ageInTicks, float headYaw, float headPitch, float scale)
    {
        ...
        rightLeg.rotateAngleX = MathHelper.Cos(yOffset * 0.6662F) * 1.4F * limbSwingAmount;
        leftLeg.rotateAngleX = MathHelper.Cos(yOffset * 0.6662F + (float)Math.PI) * 1.4F * limbSwingAmount;
