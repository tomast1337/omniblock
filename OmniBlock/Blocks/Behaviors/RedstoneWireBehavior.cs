...
public sealed class RedstoneWireBehavior(Block wire, Block[] conductors, Block repeater, Block poweredRepeater) : BlockRuntimeBehavior, IRedstoneComponent, IBlockPhysics, IBlockTicker, IBlockLifecycle, IBlockVisuals
{
    ...
    public void RandomDisplayTick(Block block, OnTickEvent @event)
    {
        ...
        var x = @event.X + 0.5D + (@event.World.Random.NextFloat() - 0.5D) * 0.2D;
        double y = @event.Y + 1.0F / 16.0F;
        var z = @event.Z + 0.5D + (@event.World.Random.NextFloat() - 0.5D) * 0.2D;
        var powerRatio = meta / 15.0F;
        var xVel = powerRatio * 0.6F + 0.4F;

        var yVel = powerRatio * powerRatio * 0.7F - 0.5F;
        var zVel = powerRatio * powerRatio * 0.6F - 0.7F;
        if (yVel < 0.0F) yVel = 0.0F;

        if (zVel < 0.0F) zVel = 0.0F;
    ...
    public bool IsPoweringSide(Block block, IBlockReader reader, int x, int y, int z, int side)
    {
        ...
        var connectsMinusX = IsPowerProviderOrWire(reader, x - 1, y, z, 1) || (!reader.ShouldSuffocate(x - 1, y, z) && IsPowerProviderOrWire(reader, x - 1, y - 1, z, -1));
        var connectsPlusX = IsPowerProviderOrWire(reader, x + 1, y, z, 3) || (!reader.ShouldSuffocate(x + 1, y, z) && IsPowerProviderOrWire(reader, x + 1, y - 1, z, -1));
        var connectsMinusZ = IsPowerProviderOrWire(reader, x, y, z - 1, 2) || (!reader.ShouldSuffocate(x, y, z - 1) && IsPowerProviderOrWire(reader, x, y - 1, z - 1, -1));
    ...
    private void UpdateAndPropagateCurrentStrength(IWorldContext level, int startX, int startY, int startZ)
    {
        ...
        foreach (var pos in updateList) level.Broadcaster.NotifyNeighbors(pos.X, pos.Y, pos.Z, wire.Id);
    }

    private void CalculateCurrentChanges(IWorldContext level, int x, int y, int z, int sourceX, int sourceY, int sourceZ)
    {
        var oldMeta = level.Reader.GetBlockMeta(x, y, z);

        s_wiresProvidePower.Value = false;
        var isIndirectlyPowered = level.Redstone.IsPowered(x, y, z);
        ...
        else
        {
            ...
            {
                var nx = x + (dir == 0 ? -1 : dir == 1 ? 1 : 0);
                var nz = z + (dir == 2 ? -1 : dir == 3 ? 1 : 0);

                if (nx != sourceX || nz != sourceZ) maxCurrent = GetMaxCurrentStrength(level.Reader, nx, y, nz, maxCurrent);

                if (level.Reader.ShouldSuffocate(nx, y, nz))
                {
                    if (!level.Reader.ShouldSuffocate(x, y + 1, z) && (nx != sourceX || nz != sourceZ)) maxCurrent = GetMaxCurrentStrength(level.Reader, nx, y + 1, nz, maxCurrent);
                }
        ...
        for (var dir = 0; dir < 4; dir++)
        {
            ...
            var neighborMax = GetMaxCurrentStrength(level.Reader, nx, y, nz, -1);
            if (neighborMax >= 0 && neighborMax != (maxCurrent > 0 ? maxCurrent - 1 : 0)) CalculateCurrentChanges(level, nx, y, nz, x, y, z);

            neighborMax = GetMaxCurrentStrength(level.Reader, nx, ny, nz, -1);
            if (neighborMax >= 0 && neighborMax != (maxCurrent > 0 ? maxCurrent - 1 : 0)) CalculateCurrentChanges(level, nx, ny, nz, x, y, z);
        }
    ...
    private int GetMaxCurrentStrength(IBlockReader reader, int x, int y, int z, int power)
    {
        ...
        var currentStrength = reader.GetBlockMeta(x, y, z);
    ...
    public bool IsPowerProviderOrWire(IBlockReader reader, int x, int y, int z, int direction)
    {
        ...
        var orientation = meta & 3;
        var opposite = (orientation + 2) & 3;
