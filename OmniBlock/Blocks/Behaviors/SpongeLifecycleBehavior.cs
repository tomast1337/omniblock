...
///     <para>
///         Absorb radius (<paramref name="absorbRadius" />) is a required, param (see <c>BehaviorRegistry</c>'s
///         <c>"sponge_lifecycle"</c> entry).
///     </para>
/// </summary>
public sealed class SpongeLifecycleBehavior(int absorbRadius) : IBlockLifecycle
{
    ...
    {
        for (var checkX = @event.X - absorbRadius; checkX <= @event.X + absorbRadius; ++checkX)
        for (var checkY = @event.Y - absorbRadius; checkY <= @event.Y + absorbRadius; ++checkY)
        for (var checkZ = @event.Z - absorbRadius; checkZ <= @event.Z + absorbRadius; ++checkZ)
