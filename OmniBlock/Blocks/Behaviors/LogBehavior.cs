...
public sealed class LogBehavior(Block canopy, int searchRadius, int top, int[] sides) : IBlockVisuals, IBlockLifecycle
{
    ...
    {
        var regionExtent = searchRadius + 1;
        if (!@event.World.ChunkHost.IsRegionLoaded(@event.X - regionExtent, @event.Y - regionExtent, @event.Z - regionExtent, @event.X + regionExtent, @event.Y + regionExtent, @event.Z + regionExtent)) return;
        ...
        {
            var neighborBlockId = @event.World.Reader.GetBlockId(@event.X + offsetX, @event.Y + offsetY, @event.Z + offsetZ);
            ...
            if ((leavesMeta & 8) == 0) @event.World.Writer.SetBlockMetaWithoutNotifyingNeighbors(@event.X + offsetX, @event.Y + offsetY, @event.Z + offsetZ, leavesMeta | 8);
        }
    }
