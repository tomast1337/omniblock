...
public interface IBlockPhysics
{
    ...
    /// </summary>
    bool CanPlaceAt(Block block, CanPlaceAtContext @event)
    {
        return true;
    }

    bool CanGrow(Block block, OnTickEvent @event)
    {
        return true;
    }
    ...
    /// </summary>
    Box? GetCollisionShape(Block block, IBlockReader reader, EntityManager entities, int x, int y, int z, Box? defaultShape)
    {
        return defaultShape;
    }
    ...
    /// </summary>
    bool HasCollision(Block block, bool defaultHasCollision)
    {
        return defaultHasCollision;
    }
    ...
    /// </summary>
    bool HasCollision(Block block, int meta, bool allowLiquids, bool defaultHasCollision)
    {
        return defaultHasCollision;
    }

    /// <summary>Whether this block can catch fire from the given neighbor position (fire's own spread registry).</summary>
    bool IsFlammable(Block block, IBlockReader reader, int x, int y, int z, bool defaultFlammable)
    {
        return defaultFlammable;
    }

    /// <summary>Overrides the velocity applied to entities standing in/on this block (e.g. fluid current).</summary>
    Vec3D ApplyVelocity(Block block, OnApplyVelocityEvent @event, Vec3D defaultVelocity)
    {
