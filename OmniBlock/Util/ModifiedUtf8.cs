...
internal static class ModifiedUtf8
{
    ...
    {
        if (TryGetString(bytes, out var result))
        {
            return result;
        }
    ...
    public static bool TryGetString(ReadOnlySpan<byte> bytes, [NotNullWhen(true)] out string? value)
    {
        ...
        if (!TryGetCharCount(bytes, out var charCount))
        {
            value = null;
            return false;
        }
        ...
        value = string.Create(charCount, bytes, (chars, stateBytes) => { GetStringInternal(stateBytes, chars); });
    ...
    {
        ref var start = ref MemoryMarshal.GetReference(str);
        ...
        if (Vector256.IsHardwareAccelerated && length >= Vector256<ushort>.Count)
        {
            ...
            {
                var v = Vector256.LoadUnsafe(ref Unsafe.As<char, ushort>(ref start), (nuint)i);
    ...
    private static bool TryGetCharCount(ReadOnlySpan<byte> bytes, out int charCount)
    {
        ...
        while (index < bytes.Length)
        {
            ...
            else if ((firstByte & 0xE0) == 0xC0)
            {
                ...
                var val = ((firstByte & 0x1F) << 6) | (secondByte & 0x3F);
            ...
            else if ((firstByte & 0xF0) == 0xE0)
            {
                ...
                var secondByte = bytes[index + 1];
                var thirdByte = bytes[index + 2];
                if ((secondByte & 0xC0) != 0x80 || (thirdByte & 0xC0) != 0x80) return false;

                var val = ((firstByte & 0x0F) << 12) | ((secondByte & 0x3F) << 6) | (thirdByte & 0x3F);
    ...
    private static void GetStringInternal(ReadOnlySpan<byte> bytes, Span<char> destination)
    {
        ...
        while (byteIndex < bytes.Length)
        {
            ...
            {
                var secondByte = bytes[byteIndex + 1];
            ...
            {
                var secondByte = bytes[byteIndex + 1];
                var thirdByte = bytes[byteIndex + 2];
