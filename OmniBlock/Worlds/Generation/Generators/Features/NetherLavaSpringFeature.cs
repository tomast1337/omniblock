...
internal class NetherLavaSpringFeature : Feature
{
    ...
    {
        if (level.Reader.GetBlockId(x, y + 1, z) != level.Content.Blocks.Get("netherrack").Id)
        {
            return false;
        }

        if (level.Reader.GetBlockId(x, y, z) != 0 && level.Reader.GetBlockId(x, y, z) != level.Content.Blocks.Get("netherrack").Id)
        {
            return false;
        }
        ...
        if (level.Reader.GetBlockId(x, y, z - 1) == level.Content.Blocks.Get("netherrack").Id)
        {
            ++netherrackNeighbors;
        }
