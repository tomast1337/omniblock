...
internal class ServerWorldEventListener : IWorldEventListener
{
    ...
    public void NotifyEntityAdded(Entity entity) => server.getEntityTracker(world.Dimension.Id).onEntityAdded(entity);

    public void NotifyEntityRemoved(Entity entity) => server.getEntityTracker(world.Dimension.Id).onEntityRemoved(entity);

    public void BlockUpdate(int x, int y, int z) => server.playerManager.markDirty(x, y, z, world.Dimension.Id);

    public void UpdateBlockEntity(int x, int y, int z, BlockEntity blockEntity) => PlayerManager.updateBlockEntity(x, y, z, blockEntity);
    ...
    public void BroadcastEntityEvent(Entity entity, byte @event)
    {
        server.getEntityTracker(world.Dimension.Id)
            .sendToAround(entity, new EntityStatusMessage
            {
                EntityId = entity.ID,
                Status = (sbyte)@event
            });
    ...
    public void PlayNote(int x, int y, int z, int soundType, int pitch)
    {
        server.playerManager.sendToAround(x, y, z, 64.0, world.Dimension.Id,
            new PlayNoteSoundMessage
            {
                X = x,
                Y = (short)y,
                Z = z,
                Instrument = (byte)soundType,
                Pitch = (byte)pitch
            });
    }
