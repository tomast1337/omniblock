...
internal sealed class PlaceBlockBehavior : IItemBehavior
{
    ...
    public bool UseOnBlock(Item item, ItemStack itemStack, EntityPlayer player, IWorldContext world, int x, int y, int z, int meta)
    {
        ...
        if (!block.CanPlaceAt(new CanPlaceAtContext(world, 0, x, y, z))) return true;

        if (!world.Writer.SetBlock(x, y, z, BlockId)) return true;

        if (block.HasBlockEntity && world.Entities.GetBlockEntity<BlockEntity>(x, y, z) is IBlockEntityItemData itemData)
        {
