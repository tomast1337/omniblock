...
internal sealed class BucketBehavior : IItemBehavior
{
    ...
    private int IsFull => _isFullFactory();
    ...
    public ItemStack Use(Item item, ItemStack itemStack, IWorldContext world, EntityPlayer player)
    {
        ...
        var hitResult = world.Reader.Raycast(rayStart, rayEnd, IsFull == 0);
        ...
        if (IsFull == 0)
        {
            ...
            if (world.Reader.GetMaterial(hitX, hitY, hitZ) != Material.Lava || world.Reader.GetBlockMeta(hitX, hitY, hitZ) != 0) return itemStack;
        ...
        if (!world.Reader.IsAir(hitX, hitY, hitZ) && world.Reader.GetMaterial(hitX, hitY, hitZ).IsSolid) return itemStack;

        if (world.Dimension.EvaporatesWater && IsFull == world.Content.Blocks.Get("omniblock:flowing_water").Id)
        {
