...
public sealed class BlockGrassTests
{
    ...
    {
        using var json = JsonDocument.Parse("""{"Type":"grass_ticker"}""");
    ...
    {
        using var json = JsonDocument.Parse("""{"Type":"grass_ticker","soil":"not_a_real_block"}""");
        Assert.Throws<KeyNotFoundException>(() => BehaviorRegistry.Build("grass_ticker", json.RootElement));
    }
    ...
    {
        using var json = JsonDocument.Parse("""{"Type":"grass_ticker","soil":"omniblock:dirt","die_light_threshold":4,"die_chance_one_in":4,"spread_light_threshold":9}""");
        var behavior = BehaviorRegistry.Build("grass_ticker", json.RootElement);
    ...
    {
        using var json = JsonDocument.Parse("""{"Type":"grass_ticker","soil":"omniblock:dirt"}""");
        Assert.Throws<KeyNotFoundException>(() => BehaviorRegistry.Build("grass_ticker", json.RootElement));
    }
}
