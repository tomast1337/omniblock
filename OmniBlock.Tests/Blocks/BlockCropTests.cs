...
public sealed class BlockCropTests
{
    ...
    public void CanPlaceAt_CustomFarmland_MatchesConfiguredBlockNotVanillaFarmland()
    {
        ...
        var customSoil = TestBlocks.Get("sand");
        var vanillaFarmland = TestBlocks.Get("farmland");
        var wheat = ContentRuntime.Current.Items.Get("omniblock:wheat");
        var seeds = ContentRuntime.Current.Items.Get("omniblock:seeds");

        world.ReaderWriter.SetInitial(0, 63, 0, customSoil.Id);
        CropBehavior behavior = new(customSoil, wheat, seeds, 0.7F, 15, 100, [0, 0, 0, 0, 0, 0, 0, 0]);
    ...
    {
        var apple = ContentRuntime.Current.Items.Get("omniblock:apple");
        CropBehavior behavior = new(TestBlocks.Get("farmland"), apple, ContentRuntime.Current.Items.Get("omniblock:seeds"), 0.7F, 15, 100, [0, 0, 0, 0, 0, 0, 0, 0]);
    ...
    {
        using var json = JsonDocument.Parse("""{"Type":"crop","mature_crop_item":"omniblock:wheat","seeds":"omniblock:seeds"}""");
    ...
    {
        using var json = JsonDocument.Parse("""{"Type":"crop","required_soil":"not_a_real_block","mature_crop_item":"omniblock:wheat","seeds":"omniblock:seeds"}""");
        Assert.Throws<KeyNotFoundException>(() => BehaviorRegistry.Build("crop", json.RootElement));
    }
    ...
    {
        using var json = JsonDocument.Parse("""{"Type":"crop","required_soil":"omniblock:farmland","mature_crop_item":"omniblock:wheat","seeds":"omniblock:seeds"}""");
        Assert.Throws<KeyNotFoundException>(() => BehaviorRegistry.Build("crop", json.RootElement));
    }
}
