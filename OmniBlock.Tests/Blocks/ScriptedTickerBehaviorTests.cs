...
public sealed class ScriptedTickerBehaviorTests
{
    ...
    public void OnTick_WithRegisteredHook_InvokesHookWithPrimitiveArgs()
    {
        ...
        world.ReaderWriter.SetInitial(1, 64, 2, BlockRegistry.Get("stone").Id, meta: 3);
        ...
        behavior.OnTick(BlockRegistry.Get("stone"),
            Tick(world, 1, 64, 2, meta: 3, blockId: BlockRegistry.Get("stone").Id));

        Assert.NotNull(captured);
        Assert.Equal((1, 64, 2, 3, BlockRegistry.Get("stone").Id), (captured!.Value.X, captured.Value.Y, captured.Value.Z, captured.Value.Meta, captured.Value.BlockId));
    }
    ...
    {
        FakeWorldContext world = new();
        world.ReaderWriter.SetInitial(5, 64, 5, BlockRegistry.Get("stone").Id, meta: 0);

        ScriptTickHookRegistry.Register("test:mutate_hook", (host, x, y, z, _, _) =>
        {
            Assert.Equal(BlockRegistry.Get("stone").Id, host.GetBlockId(x, y, z));
            host.SetBlock(x, y, z, BlockRegistry.Get("dirt").Id);
            host.SetBlockMeta(x, y, z, 7);
        });

        ScriptedTickerBehavior behavior = new("test:mutate_hook");
        behavior.OnTick(BlockRegistry.Get("stone"),
            Tick(world, 5, 64, 5, meta: 0, blockId: BlockRegistry.Get("stone").Id));

        Assert.Equal(BlockRegistry.Get("dirt").Id, world.Reader.GetBlockId(5, 64, 5));
    ...
    {
        FakeWorldContext world = new();
        world.ReaderWriter.SetInitial(0, 64, 0, BlockRegistry.Get("stone").Id);

        ScriptedTickerBehavior behavior = new("test:never_registered_hook_key");

        behavior.OnTick(BlockRegistry.Get("stone"),
            Tick(world, 0, 64, 0, meta: 0, blockId: BlockRegistry.Get("stone").Id));

        Assert.Equal(BlockRegistry.Get("stone").Id, world.Reader.GetBlockId(0, 64, 0));
    }
}
