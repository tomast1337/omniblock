...
public class TestWorldSaveStorage
{
    ...
    {
        var baseDir = Path.Combine(Path.GetTempPath(), "OmniBlockTestWorldPerPlayer");
        if (Directory.Exists(baseDir)) Directory.Delete(baseDir, true);

        var storage = new RegionWorldStorage(baseDir, "world", true);
        FakeWorldContext world = new();

        var alice = new TestEntityPlayer(world)
        {
            Name = "Alice"
        };
        ...
        var bob = new TestEntityPlayer(world)
        {
            Name = "Bob"
        };
        ...
        var aliceReload = new TestEntityPlayer(world)
        {
            Name = "Alice"
        };
        ...
        var bobReload = new TestEntityPlayer(world)
        {
            Name = "Bob"
        };
    ...
    {
        var baseDir = Path.Combine(Path.GetTempPath(), "OmniBlockTestWorld");
        if (Directory.Exists(baseDir)) Directory.Delete(baseDir, true);
        ...
        var levelDat = Path.Combine(baseDir, "world", "level.dat");
