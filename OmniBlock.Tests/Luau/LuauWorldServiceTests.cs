...
public sealed class LuauWorldServiceTests
{
    ...
    public void ListsSavesAndQueuesSupportedWorldWithStoredSettings()
    {
        ...
        Assert.True(service.TryTakePending(out var request));
    ...
    public void RejectsMissingAndUnsupportedWorlds(string id) => Assert.False(new LuauWorldService(new StubStorage(), () => true).RequestLoad(id));

    [Fact]
    public void RejectsLoadsOutsideSafeMainMenuState() => Assert.False(new LuauWorldService(new StubStorage(), () => false).RequestLoad("World1"));
