...
public sealed class LuauCommandHistoryTests
{
    ...
    public void Navigation_walksCommandsAndRestoresDraft()
    {
        ...
        Assert.Equal("third()", history.Navigate("unfinished", true));
        Assert.Equal("first()\nsecond()", history.Navigate("third()", true));
        Assert.Equal("first()\nsecond()", history.Navigate("first()\nsecond()", true));
        Assert.Equal("third()", history.Navigate("first()\nsecond()", false));
        Assert.Equal("unfinished", history.Navigate("third()", false));
    }
    ...
    public void History_isBoundedAndSkipsConsecutiveDuplicates()
    {
        ...
        Assert.Equal("three", history.Navigate("", true));
        Assert.Equal("two", history.Navigate("three", true));
        Assert.Equal("two", history.Navigate("two", true));
    }

    [Fact]
    public void History_persistsAsEscapedPlainTextAndReloadsWholeCommands()
    {
        var directory = Path.Combine(Path.GetTempPath(), "omniblock-history-tests", Guid.NewGuid().ToString("N"));
        var path = Path.Combine(directory, ".luau_history");
        try
        {
            ...
            Assert.Equal("return 3", reader.Navigate("", true));
            Assert.Equal("print(\\\"literal \\\\n\\\")\nprint(2)", reader.Navigate("return 3", true));
        }
        finally
        {
