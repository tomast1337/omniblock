...
public sealed class PathMoverBufferTests
{
    ...
    public void Spawn_setsStartEndAndInitialPositionToStart()
    {
        ...
        var i = buf.Spawn(0, 0, 0, 10, 0, 0, 1.0, 5, 1.0f);
    ...
    {
        PathMoverBuffer buf = new();
        var i = buf.Spawn(0, 0, 0, 10, 0, 0, 2.0, 0, 1.0f);

        Assert.Equal(0.2f, buf.ProgressPerTick[i], 5);
    }
    ...
    {
        PathMoverBuffer buf = new();
        buf.Spawn(0, 0, 0, 10, 20, 0, 1.0, 0, 1.0f);

        PathMoverUpdater.Tick(buf);

        double progress = buf.Progress[0];
        Assert.Equal(10.0 * progress, buf.X[0], 6);
        Assert.Equal(20.0 * progress, buf.Y[0], 6);
    ...
    {
        PathMoverBuffer buf = new();
        buf.Spawn(0, 0, 0, 1, 0, 0, 10.0, 0, 1.0f);
        Assert.Equal(1, buf.Count);
    ...
    {
        PathMoverBuffer buf = new();
        buf.Spawn(3, 4, 5, 3, 4, 5, 1.0, 0, 1.0f);
        Assert.Equal(1, buf.Count);
    ...
    {
        PathMoverBuffer buf = new();
        buf.Spawn(0, 0, 0, 100, 0, 0, 1.0, 0, 1.0f);

        var xBeforeTick = buf.X[0];
        PathMoverUpdater.Tick(buf);
