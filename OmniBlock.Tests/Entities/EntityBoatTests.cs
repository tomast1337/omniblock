...
public sealed class EntityBoatTests
{
    ...
    {
        var boat = BoatBehavior.Launch(world, x, y, z);
    ...
    {
        Assert.True(ContentRuntime.Current.Items.TryParse(itemName, out var resolved));
        var itemId = resolved.ItemId;
    ...
    public void An_occupied_boat_refuses_another_player()
    {
        FakeWorldContext world = new();
        var boat = Launch(world);
        var first = Sailor(world);
        TestEntityPlayer second = new(world)
        {
            Name = "stowaway"
        };
    ...
    public void A_hit_rocks_the_hull_the_other_way_each_time()
    {
        ...
        var restingDirection = Hull.RockDirection(boat);
    ...
    public void A_submerged_boat_is_pushed_back_up()
    {
        ...
        for (var x = 7; x <= 10; x++)
        {
            for (var z = 7; z <= 10; z++) EntityTestHarness.FillWaterColumn(world, x, z, 61, 68);
        }

        var boat = Launch(world, y: 64.0);
        boat.VelocityY = -0.3;
        boat.Tick();
    ...
    public void A_boat_ploughs_through_snow_rather_than_riding_over_it()
    {
        FakeWorldContext world = new();
        EntityTestHarness.PlaceStoneFloor(world, 0, 15, 0, 15, 63);
        var snowId = TestBlocks.Get("snow").Id;
        for (var x = 7; x <= 10; x++)
        {
            for (var z = 7; z <= 10; z++) world.Writer.SetBlock(x, 64, z, snowId);
        }
    ...
    {
        var boat = TestEntityCatalog.ByName("boat").RequireDefinition();
