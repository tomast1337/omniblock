...
public sealed class BlockBatch7Tests
{
    ...
    public void RedstoneWireBehavior_IsPowerProviderOrWire_VanillaLeverNotInCustomConfig_ChecksCanEmitInstead()
    {
        ...
        // Block.Blocks[id].canEmitRedstonePower(), which is independently true for levers.
        Assert.True(behavior.IsPowerProviderOrWire(world.Reader, 0, 64, 0, -1));
    }

    [Theory]
    [InlineData("dispenser", """{"Type":"dispenser","egg":"egg","snowball":"snowball"}""")]
    [InlineData("portal", """{"Type":"portal"}""")]
    [InlineData("redstone_wire", """{"Type":"redstone_wire","conductors":["button"],"repeater":"repeater","powered_repeater":"powered_repeater"}""")]
