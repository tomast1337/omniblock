...
public sealed class MessageGenerator : IIncrementalGenerator
{
    ...
    private static bool InheritsMessage(INamedTypeSymbol type)
    {
        ...
        {
            if (current.ToDisplayString() == "BetaSharp.Network.Messages.Message")
            {
                return true;
            }
        }
    ...
    private static string Emit(MessageModel model)
    {
        ...
        source.AppendLine();
        source.AppendLine("using BetaSharp;");
        source.AppendLine();
        ...
        source.AppendLine(
            $"    public static readonly global::BetaSharp.ResourceLocation Id = "
            + $"new(global::BetaSharp.Namespace.Get(\"{model.KeyNamespace}\"), \"{model.KeyPath}\");");
        source.AppendLine();
        source.AppendLine("    public override global::BetaSharp.ResourceLocation Key => Id;");
        source.AppendLine();
    ...
    private static string EmitRegistrations(ImmutableArray<MessageModel?> models)
    {
        ...
        source.AppendLine("#nullable enable");
        source.AppendLine();
        source.AppendLine("namespace BetaSharp.Network.Messages;");
        source.AppendLine();
