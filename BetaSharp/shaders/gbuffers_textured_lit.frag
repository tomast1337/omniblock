...
// Identical to gbuffers_textured's, and deliberately a separate file rather than a shared include:
// this is the stage a pack overrides to relight the world, so it has to be somewhere a pack can
// replace one of the two without replacing both.
...
void main()
{
    ...
    vec4 color = tint * texture(textureSampler, texCoord);
