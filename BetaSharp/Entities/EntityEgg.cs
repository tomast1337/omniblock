...
public sealed class EntityEgg : Entity
{
    ...
    public override void Tick()
    {
        ...
        if (!World.IsRemote)
        {
            ...
            {
                if (!entity.HasCollision || (Equals(entity, _thrower) && _ticksInAir < 5))
                {
                    continue;
                }
    ...
    {
        if (!_inGround || !Equals(_thrower, player) || _shake > 0 || !player.Inventory.AddItemStackToInventory(new ItemStack(Item.ByName("arrow"), 1))) return;
