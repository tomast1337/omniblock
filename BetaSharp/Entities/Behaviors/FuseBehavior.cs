...
/// <summary>
///     A creeper's fuse: it winds up while a target is in range, winds back down when the target is
///     lost or hidden, and detonates at full charge. One behavior across the Attack, Ticker and
///     Lifecycle slots, because every one of those hooks moves the same countdown.
///     <para>
...
public sealed class FuseBehavior : IEntityAttackBehavior, IEntityTicker, IEntityLifecycle
{
    ...
    public FuseBehavior(in EntityBehaviorContext context)
    {
        ...
        _fuseSound = context.Json.TryGetProperty("fuse_sound", out System.Text.Json.JsonElement s)
    ...
    /// <summary>A creeper that has lost its target lets the fuse burn back down.</summary>
    public void OnTickEnd(EntityLiving self)
    {
        if (self.World.IsRemote || self is not EntityCreature { Target: null }) return;
        if (self.State[_charge] <= 0) return;
    ...
    /// <summary>Target behind cover: the fuse burns down rather than holding its charge.</summary>
    public void AttackBlockedEntity(EntityCreature self, Entity target, float distance)
    {
        if (self.World.IsRemote || self.State[_charge] <= 0) return;
