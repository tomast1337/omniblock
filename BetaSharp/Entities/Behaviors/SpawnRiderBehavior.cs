...
/// <summary>
///     Occasionally spawns another entity riding this one — the skeleton that rides a spider. Both
///     the rider and the odds are declared, so the pairing is data rather than a hardcoded class.
/// </summary>
public sealed class SpawnRiderBehavior(string rider, int chanceOneIn) : IEntityLifecycle
{
    ...
    {
        if (self.World.Random.NextInt(chanceOneIn) != 0) return;
