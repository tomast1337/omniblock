...
/// <summary>
///     The chest a chest minecart carries. It is its own object rather than something the cart
///     entity implements, because only one of the three cart types has one and
///     <see cref="EntityObject" /> is shared by every non-living entity in the game.
/// </summary>
public sealed class MinecartCargo(Entity cart) : IInventory
{
    ...
    {
        if (_slots[slotIndex] is not { } stack) return null;
        ...
        if (stack.Count == 0) _slots[slotIndex] = null;
