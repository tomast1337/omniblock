...
/// <summary>
///     Cocks the mob's head at a player holding something it wants — the bribe while it is still
///     wild, any meat once it is tamed — and holds its gaze while the offer stands.
/// </summary>
public sealed class HeadTiltBehavior : IEntityPhysics, IEntityTicker
{
    ...
    public void AfterTickMovement(EntityLiving self)
    {
        ...
        if (self is not EntityCreature mob || !mob.HasCurrentTarget || mob.HasPath) return;
        if (mob.Behaviors.Find<TameableBehavior>() is not { } tame || tame.IsAngry(mob)) return;
        if (mob.CurrentTarget is not EntityPlayer watched) return;
    ...
    public void OnTickEnd(EntityLiving self)
    {
        ...
        // Keep watching while the offer stands, rather than glancing away mid-tilt.
        if (state[_interested]) self.HoldGaze(_holdGazeTicks);
    }
