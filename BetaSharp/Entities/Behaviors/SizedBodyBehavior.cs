...
/// <summary>
///     A mob that comes in sizes, where the size <em>is</em> the body: it sets the bounding box and
///     the health, and everything else about the mob scales off it. The size is synced because the
///     client draws the mob from it, and persisted with an offset because the on-disk value counts
///     from zero while the live one counts from one.
///     <para>
///         Size is rolled once at creation, so a mob split off a bigger one or loaded from disk gets
///         a throwaway roll first and is then set properly — the same two steps the constructor took.
///     </para>
/// </summary>
public sealed class SizedBodyBehavior : IEntityLifecycle, IEntityPersistence
{
    ...
    {
        List<int> choices = [];
        foreach (JsonElement choice in context.Json.GetProperty("sizes").EnumerateArray()) choices.Add(choice.GetInt32());
        _choices = [.. choices];
    ...
    {
        if (self is EntityLiving mob) SetSize(mob, nbt.GetInteger(_nbtKey) - _nbtOffset);
    }
}
