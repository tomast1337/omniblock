...
/// <summary>
///     A small thrown projectile: it flies on a shallow arc, remembers who threw it so it cannot
///     hit them point-blank, and pops on whatever it touches first — block or entity. The snowball
///     and the egg are the same flight verbatim; the egg adds a hatch roll on impact, declared as
///     data.
///     <para>
///         Which mob it was thrown by is per-instance state set by <see cref="Throw" />, the one
///         spawner for a hand-thrown projectile; a dispenser creates the entity bare and aims it
///         with <see cref="SetHeading" />.
///     </para>
/// </summary>
public sealed class ThrownProjectileBehavior : IEntityTicker, IEntityPersistence, IEntityInteractable, IEntityPhysics
{
    ...
    /// <summary>The egg's hatch roll, absent for the snowball: on impact, a 1-in-<c>Chance</c>
    ...
    /// <summary>
    ///     The hand-thrown spawner: positions the projectile at the thrower's eyes, nudged back so
    ///     it does not clip their face, and launches it the way they are looking. The caller spawns
    ///     the returned entity.
    /// </summary>
    ...
    public bool OnTickEntity(Entity self)
    {
        ...
        if (!self.World.IsRemote)
        {
            ...
            {
                if (!entity.HasCollision || (Equals(entity, thrower) && self.State[_ticksInAir] < 5)) continue;
    ...
    /// <summary>
    ///     A verbatim quirk of the beta classes, which copied the arrow's pickup code: a projectile
    ///     stuck in the ground hands its thrower an arrow. Unreachable in practice — flight never
    ///     sets the in-ground flag, and the thrower does not survive a save — but ported as found.
    /// </summary>
