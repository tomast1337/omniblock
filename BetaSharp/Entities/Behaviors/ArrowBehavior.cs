...
///     player-owned arrow stuck in the ground can be pulled back out. Two statics answer what the
///     rest of the game asks of it: <see cref="IsArrow" /> for the damage-scaling rules and
///     <see cref="OwnerOf" /> to credit the hit to whoever fired it.
/// </summary>
public sealed class ArrowBehavior : IEntityTicker, IEntityPersistence, IEntityInteractable, IEntityPhysics
{
    ...
    /// <summary>Whether this entity is an arrow.</summary>
    public static bool IsArrow(Entity? entity) => entity?.Behaviors.Find<ArrowBehavior>() is not null;

    /// <summary>Who fired this arrow, or null when it is not an arrow or nobody owns it.</summary>
    public static EntityLiving? OwnerOf(Entity? entity) =>
        entity?.Behaviors.Find<ArrowBehavior>() is { } arrow ? arrow.Owner(entity) : null;

    /// <summary>
    ///     Positions the arrow at the shooter's eyes, nudged back so it does not clip their face,
    ///     and fires it the way they are looking. The caller spawns the returned entity, and a mob
    ///     re-aims it with <see cref="SetHeading" /> after.
    /// </summary>
