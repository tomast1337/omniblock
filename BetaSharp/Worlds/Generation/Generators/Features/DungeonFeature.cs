...
internal class DungeonFeature : Feature
{
    public override bool Generate(IWorldContext level, JavaRandom rand, int x, int y, int z)
    {
            ...
            for (int cy = y + height; cy >= y - 1; --cy)
            {
                for (int cz = z - radiusZ - 1; cz <= z + radiusZ + 1; ++cz)
                {
                    ...
                    else if (level.Reader.GetMaterial(cx, cy, cz).IsSolid)
                    {
                        ...
                        {
                            level.Writer.SetBlock(cx, cy, cz, BlockRegistry.Get("mossy_cobblestone").id, 0, false);
                        }
                        else
                        {
                            level.Writer.SetBlock(cx, cy, cz, BlockRegistry.Get("cobblestone").id, 0, false);
                        }
                    }
                }
            }
        ...
        for (int i = 0; i < 2; ++i)
        {
            for (int j = 0; j < 3; ++j)
            {
                ...
                if (level.Reader.IsAir(chestX, y, chestZ))
                {
                    ...
                    level.Writer.SetBlock(chestX, y, chestZ, BlockRegistry.Get("chest").id, 0, true);
        ...
        level.Writer.SetBlock(x, y, z, BlockRegistry.Get("spawner").id, 0, true);
