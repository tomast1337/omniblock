            ...
            if ((blockId != 0 && blockId != Block.Leaves.Id) || y <= 0)
            {
                for (int i = 0; i < 4; ++i)
                {
                    ...
                    if (level.Reader.IsAir(genX, genY, genZ) &&
                        Block.Blocks[_deadBushBlockId].CanGrow(new OnTickEvent(level, genX, genY, genZ, level.Reader.GetBlockMeta(genX, genY, genZ), level.Reader.GetBlockId(genX, genY, genZ))))
                    {
