...
public class Chunk
{
    ...
    private void UpdateHeightMap(int localX, int y, int localZ)
    {
        ...
        // The height is recomputed on a remote world too, because the renderer needs it and the
        // wire carries no heightmap. The light that follows from it is not: a remote world holds
        // the sky the server wrote, and the section snapshot that follows a change overwrites
        // anything computed here. Writing it locally would race that snapshot.
        if (!World.IsRemote)
        {
